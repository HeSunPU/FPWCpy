...
if __name__ == "__main__":
	...
	params_values['Q1'] = 0.10 # 0.05 # 0.5 # 1e-8 for u^2, 6e-8 for u^3, 0.5 for dE^2
	...
	for trial in range(n_trials):
		...
		for k in range(Nitr):
			...
			# camera.set_exposure(np.max([0.5e-9/contrast[k], 1e-2]))
			...
			# u_p_values = sensor.Probe_command(u_p_values, exp_time, contrast[k], rate=5e-4, beta=0., gamma=1., Nitr=1000, print_flag=True)
